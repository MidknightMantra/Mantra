...
import { cacheMessage, getCachedMessage } from '../src/utils/messageCache.js';
...
export async function initListeners(conn, store) {
    console.log(chalk.hex('#6A0DAD')("🔮 Listeners Initialized: AntiDelete (with caching), AntiViewOnce, AutoStatus"));
    ...
    conn.ev.on('messages.upsert', async (chatUpdate) => {
        try {
            for (const msg of chatUpdate.messages) {
                ...
                // Cache every incoming message for anti-delete (24h TTL)
                const remoteJid = msg.key.remoteJid;
                const messageId = msg.key.id;
                if (messageId && remoteJid) {
                    await cacheMessage(messageId, remoteJid, msg, 86400);
                }
                ...
                if (remoteJid === 'status@broadcast') {
                    try {
                        ...
                        // CRITICAL: Send read receipt to the contact who posted the status
                        // This makes the bot appear in their status viewers list
                        const statusPoster = msg.key.participant;
                        if (statusPoster) {
                            await conn.readMessages([{
                                remoteJid: statusPoster,
                                id: msg.key.id,
                                participant: statusPoster
                            }]);
                            console.log(chalk.green(`[AUTO-STATUS] ✅ Viewed status from ${statusPoster.split('@')[0]}`));
                        }
                    } catch (e) {
                        console.error(chalk.red(`[AUTO-STATUS] Error:`, e.message));
                    }
                    continue;
                }
                ...
                if (mtype === 'protocolMessage' && msg.message.protocolMessage.type === 0) {
                    ...
                    try {
                        // Try cache first (faster and more reliable)
                        let originalMsg = await getCachedMessage(deletedId, remoteJid);
                        let source = 'cache';

                        // Fallback to store if not in cache
                        if (!originalMsg) {
                            originalMsg = await store.loadMessage(remoteJid, deletedId);
                            source = 'store';
                        }

                        console.log(chalk.cyan(`[ANTI-DELETE] Retrieved from ${source}`));

                        if (originalMsg && originalMsg.message) {
                            const sender = originalMsg.sender || originalMsg.key?.participant || originalMsg.key?.remoteJid;
                            const chatType = remoteJid.includes('@g.us') ? 'Group' : 'DM';

                            console.log(chalk.green(`[ANTI-DELETE] Found original message from ${sender} (source: ${source})`));
