 ...
 */
export async function clearAllSudo() {
 ...
 * Update sudo numbers
 */
export async function setSudo(num) {
  const current = await getSudoNumbers();
...
export async function delSudo(num) {
  const current = await getSudoNumbers();
 ...
 */
export async function incrementCommandStats(command, userJid) {
  try {
    if (useMongoDb) {
      // Update Global Stats
 ...
 */
export async function getAnalyticsStats(userJid = null) {
  try {
    if (useMongoDb) {
