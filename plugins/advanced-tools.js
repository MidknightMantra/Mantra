...
async function handleScreenshot(m, { conn, args, text }, type) {
    const url = text?.trim();
    if (!url) return m.reply(`❌ *Usage:* ${global.prefix}${type} <url>\n*Example:* .${type} https://google.com`);
    ...
    handler: async (m, { conn, text }) => {
        ...
        await withReaction(conn, m, '📲', async () => {
            try {
                ...
                await conn.sendMessage(m.chat, {
                    image: Buffer.from(res.data),
                    caption: `✨ *Mantra QR Code*\n${global.divider}\n📝 *Data:* ${content.substring(0, 100)}${content.length > 100 ? '...' : ''}`
                }, { quoted: m });
            } catch (error) {
...
addCommand({
    ...
    category: 'tools',
    handler: async (m, { conn, text }) => {
        if (!m.quoted) return m.reply('❌ Please reply to a document or media file.');

        const newName = text?.trim();
        if (!newName) return m.reply('❌ Please provide a new filename.\n\n*Example:* .rename my_video.mp4');
...
addCommand({
    ...
    handler: async (m, { conn, text, commandName }) => {
        ...
        if (!input) return m.reply(`❌ *Usage:* .fancy <text>\n*Example:* .fancy Mantra Bot`);
            ...
            try {
                ...
                if (numMatch) {
                    ...
                    if (index >= 0 && index < results.length) {
                        ...
                        return m.reply(`✨ *Style:* ${selected.name}\n\n${selected.result}`);
                    }
                }

                // Default list view
                let txt = `✨ *Mantra Fancy Fonts*\n${global.divider}\n📝 *Input:* ${input}\n\n`;
                results.forEach((r, i) => {
                    txt += `*${i + 1}.* ${r.result}\n`;
                });
                txt += `\n💡 Tip: Use .fancy<number> to copy a style\nExample: .fancy5 ${input}`;
...
addCommand({
    ...
    handler: async (m, { conn, text }) => {
        ...
        await withReaction(conn, m, '📖', async () => {
            ...
            const txt = `✨ *Mantra Dictionary*\n${global.divider}\n📖 *Word:* ${def.word}\n\n📝 *Definition:* ${def.definition.replace(/\[|\]/g, '')}\n\n💬 *Example:* _"${def.example?.replace(/\[|\]/g, '') || 'N/A'}"_\n\n👤 *Author:* ${def.author}`;
            await m.reply(txt);
        });
    }
});
...
addCommand({
    ...
    category: 'tools',
    handler: async (m, { conn, text }) => {
        if (!text) return m.reply('❌ Use: .mix 😂🙄');
        await withReaction(conn, m, '😀', async () => {
            ...
            await conn.sendMessage(m.chat, { image: Buffer.from(res.data), caption: `✨ *Mantra Emoji Mix*\n${emojis[0]} + ${emojis[1]}` }, { quoted: m });
        });
    }
});
